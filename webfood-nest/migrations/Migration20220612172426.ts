...
export class Migration20220612172426 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `user_address` (`id` integer not null primary key autoincrement, `user_id` integer not null, `zip_code` integer not null, `city` text not null, `street` text not null, `house_number` integer not null, `staircase` integer not null, `doorbell` integer not null, `floor` integer not null, `door_number` integer not null, `note` text not null, constraint `user_address_user_id_foreign` foreign key(`user_id`) references `user`(`id`) on update cascade);');
    ...
    this.addSql('create table `order` (`order_id` integer not null primary key autoincrement, `order_date` datetime not null, `user_id` integer not null, `user_address_id` integer not null, `order_status` integer not null, `restaurant_id` integer not null, constraint `order_user_id_foreign` foreign key(`user_id`) references `user`(`id`) on update cascade, constraint `order_user_address_id_foreign` foreign key(`user_address_id`) references `user_address`(`id`) on update cascade, constraint `order_restaurant_id_foreign` foreign key(`restaurant_id`) references `restaurant`(`id`) on update cascade);');
